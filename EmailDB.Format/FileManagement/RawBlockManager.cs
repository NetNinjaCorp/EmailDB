...
public class RawBlockManager : IDisposable
{
    ...
    public const int HeaderChecksumSize = 16;
    public const int PayloadChecksumSize = 16;
    ...
    public RawBlockManager(string filePath, bool createIfNotExists = true, Action<string>? logger = null)
    {
        ...
        ScanExistingBlocksAsync().GetAwaiter().GetResult();
    }
    ...
    /// <summary>
    /// Returns the Block ID of the latest Metadata block found during file scan.
    /// Returns -1 if no Metadata block has been found.
    /// </summary>
    public long GetLatestMetadataBlockId() => latestMetadataBlockId;
    ...
    /// </summary>
    /// <param name="reEncrypt">When true, re-encrypts all blocks with the active DEK during compaction. Default is false (normal space reclamation only).</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    public async Task CompactAsync(bool reEncrypt = false, CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        ...
        try
        {
            ...
            using (var tempManager = new RawBlockManager(tempFilePath)) // Use RawBlockManager for temp file
            {
                ...
                foreach (var kvp in blockLocations)
                {
                    ...
                    else
                    {
                        ...
                        _logger?.Invoke($"Compaction warning: Failed to read block {kvp.Key}, skipping. Error: {blockResult.Error}");
                        // Depending on requirements, might want to throw or handle differently
                    }
                }
            }

            // Close the current stream before replacing the file
            fileStream.Flush();
            fileStream.Dispose();
            ...
            Interlocked.Exchange(ref currentPosition, fileStream.Length);
        }
        finally
        {
    ...
    private async Task<List<long>> FindMagicPositions()
    {
        ...
        {
            _logger?.Invoke($"Error during magic position scan: {ex.Message}");
        }
        finally
        {
    ...
    /// </summary>
    private async Task ScanExistingBlocksAsync()
    {
       ...
       await fileLock.AcquireReaderLock();
        try
        {
            ...
            while (currentScanPosition < fileLength)
            {
                ...
                byte[] storedHeaderChecksum = ReadChecksumFromFileStream(fileStream);

                // Verify magic and checksum before proceeding
                ulong headerMagic = BitConverter.ToUInt64(potentialHeader, 0);
                byte[] computedHeaderChecksum = ComputeChecksum(potentialHeader);

                if (headerMagic == HEADER_MAGIC && storedHeaderChecksum.SequenceEqual(computedHeaderChecksum))
                {
        ...
        catch (Exception ex)
        {
            ...
            _logger?.Invoke($"Error during file scan: {ex.Message}");
    ...
    // Helper to read 16-byte BLAKE3 checksum directly from filestream (assumes stream position is correct)
    private byte[] ReadChecksumFromFileStream(FileStream fs)
    {
        byte[] buffer = new byte[16];
        int bytesRead = fs.Read(buffer, 0, 16);
        if (bytesRead < 16) throw new EndOfStreamException("Could not read checksum from stream.");
        return buffer;
    }
    ...
    private bool TryReadBlockLocation(MemoryMappedViewAccessor accessor, long position, long fileLength, out BlockLocation location, out long blockId)
    {
        ...
        // Read header checksum (16 bytes)
        byte[] storedHeaderChecksum = new byte[16];
        accessor.ReadArray(position + HeaderSize, storedHeaderChecksum, 0, 16);
        byte[] computedHeaderChecksum = ComputeChecksum(headerBytes);

        if (!storedHeaderChecksum.SequenceEqual(computedHeaderChecksum))
        {
            return false;
        }
    ...
    private void WriteBlockToStream(Stream stream, Block block)
    {
        ...
        using (var headerStream = new MemoryStream())
        {
            ...
            byte[] headerChecksum = ComputeChecksum(headerBytes);
        ...
        if (block.Payload != null && block.Payload.Length > 0)
        {
            ...
            byte[] payloadChecksum = ComputeChecksum(block.Payload);
    ...
    private Result<Block> ReadBlockFromStreamInternal(Stream stream, long blockIdForErrorMessage)
    {
        ...
        using (var headerReader = new BinaryReader(headerStream))
        {
            ...
            if (payloadLength > 0)
            {
                ...
                byte[] computedPayloadChecksum = ComputeChecksum(block.Payload);
                if (!storedPayloadChecksum.SequenceEqual(computedPayloadChecksum))
                {
            ...
            {
                byte[] storedPayloadChecksum = reader.ReadBytes(16); // Read the checksum for empty payload
                if (!storedPayloadChecksum.SequenceEqual(new byte[16])) // Checksum for empty payload must be 16 zero bytes
                {
    ...
    internal static byte[] ComputeChecksum(byte[] data)
    {
        var fullHash = Hasher.Hash(data).AsSpan();
        return fullHash.Slice(0, 16).ToArray();
    }

    internal static byte[] ComputeChecksum(byte[] data, int offset, int count)
    {
        var fullHash = Hasher.Hash(data.AsSpan(offset, count)).AsSpan();
        return fullHash.Slice(0, 16).ToArray();
    }
