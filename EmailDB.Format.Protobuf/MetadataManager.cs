...
using EmailDB.Format.Models; // For Block, BlockType
using ProtoBuf; // For protobuf-net serialization
using System;
using System.IO;
using System.Threading;
using System.Threading.Tasks;
using MetadataPayload = EmailDB.Format.Protobuf.Models.MetadataPayload;
...
public class MetadataManager
{
    ...
    public async Task<Result<long>> GetNextBlockIdAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                using var ms = new MemoryStream(currentMetadataBlock.Payload);
                currentPayload = Serializer.Deserialize<MetadataPayload>(ms);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                Serializer.Serialize(ms, nextPayload);
                nextPayloadBytes = ms.ToArray();
            }
            catch (Exception ex)
            {
